...
class CEILDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # Map blended filename (e.g. 2008_000973-input.png) to transmission filename (e.g. 2008_000973-label2.png)
        # t_fn = fn.replace('-input.png', '-label2.png')
        # t_path = join(self.datadir, 'transmission_layer', t_fn)
        # m_path = join(self.datadir, 'blended', fn)
...
class CEILTestDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        # Map the blended filename (*-input.png) to the transmission filename (*-label2.png)
        t_fn = fn.replace('-input.png', '-label2.png')
        t_path = join(self.datadir, 'transmission_layer', t_fn)
        m_path = join(self.datadir, 'blended', fn)
        ...
        if not os.path.exists(t_path):
            raise FileNotFoundError(f"[!] Transmission file not found: {t_path}")
